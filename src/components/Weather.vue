<template>
  <div id="page">
    ...
    <div class='content'>
      ...
      <div class='today'>Sky: {{ info.weather[0].description }}</div>
      <div>
        <img v-bind:src="`./src/assets/weather/svg/${icon}`" alt="Image">
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      icon() {
        return this.getIcon(this.info.weather[0].description)
      }
    },
    methods: {
      getIcon(value) {
        let iconsTab = [
          {'clear sky': '010-sun.svg'},
          {'wind sign': '005-wind-sign.svg'} /* exemple */
        ];

        return iconsTab.find(icon => value)[value];
      }
    }
  }
